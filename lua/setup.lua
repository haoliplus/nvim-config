...
if
  ...
  or vim.fn.executable(vim.fn.getenv("HOME") .. "/.local/share/mise/shims/python") == 1
then
  if vim.g.is_win then
    vim.g.python3_host_prog = vim.fn.substitute(vim.fn.exepath("python3"), ".exe$", "", "g")
  elseif vim.g.is_linux or vim.g.is_mac then
    if vim.g.is_linux then
      if vim.fn.executable(vim.fn.getenv("HOME") .. "/.local/share/mise/shims/python") == 1 then
        vim.g.python3_host_prog = vim.fn.exepath(vim.fn.getenv("HOME") .. "/.local/share/mise/shims/python")
